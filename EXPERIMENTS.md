# Wordle GPT — Experiment Log

Detailed training logs, ablation studies, hyperparameter sweeps, and benchmark results across iterations.
...
### 2026-09-23 — One-update GRPO check from original 7.2M SFT

Implemented one-guess GRPO in `train_grpo.py`, reachable state generation in
`grpo_states.py`, and raw/constrained gameplay diagnostics in `grpo_evaluate.py`.
Source: `runs/scaling-dev-1m/seed-0/fold-1/7.2m/checkpoints/best.pt`,
verified as 7,162,403 parameters with SFT metadata, not a DPO checkpoint.

Executed:

```bash
uv run --with-requirements requirements.txt python train_grpo.py \
  --updates 1 --states-per-update 16 --output-dir runs/grpo-dev-one-step
```

Configuration: eight distinct token-constrained guesses/state, natural-log
actual candidate reduction plus solve bonus 5, LR `1e-6`, frozen SFT reference,
raw-prefix categorical KL coefficient `0.02`. Fresh states use mixed random,
candidate, informative, entropy, poor, and partly-random reachable histories
with requested depths 0–5. Candidate counts use the full dictionary, but
sampled training secrets use only the development training split.

| Metric | SFT (0 updates) | GRPO (1 update) |
| --- | ---: | ---: |
| Constrained validation wins | 59/72 (81.94%) | 59/72 (81.94%) |
| Constrained average attempts, all games | 3.9167 | 3.9167 |
| Raw validation wins | 57/72 (79.17%) | 57/72 (79.17%) |
| Raw invalid guesses | 5 | 5 |
| Constrained mean log candidate reduction | 1.679454 | 1.679454 |
| Constrained mean action reward | 2.725553 | 2.725553 |
| Constrained expected-survivor action regret | 0.043509 | 0.043509 |

The 16 training groups (128 actions) had mean reward 1.428011, overall reward
standard deviation 1.827542, mean within-group reward standard deviation
1.280339, and 6.25% identical-reward groups (1/16). Mean fractional candidate
reduction was 0.332601; mean log reduction was 0.998323. Post-update raw-prefix
KL from SFT was `5.3795e-6`; constrained-prefix KL was `7.2943e-6`.
These KL measurements are on sampled training prefixes, not held-out game KL.
Pre-clipping gradient norm was 3.119522. Training plus both evaluations took
8.65 seconds, excluding process/model startup.

No validation gameplay improvement after one update; do not infer learning
quality from this smoke run. `checkpoints/update-1.pt` contains the updated
policy; `best.pt` retains update 0 because validation selection tied.
The 10k/25k/50k/100k schedule is implemented but was not run. No test secrets
were evaluated. Artifacts are under `runs/grpo-dev-one-step/`.

Verification: 26 tests passed across `test_train_grpo`, `test_grpo_states`,
`test_wordle`, and `test_evaluate_v2`; actual CUDA training and full 72-secret
validation completed before and after the update.

### 2026-09-23 — Dense 10,000-update GRPO run

Completed the first proper GRPO run from the original 7.2M SFT, not the
one-update GRPO model and not DPO:
...
Reward remained exactly `ln(before / after) + 5 if solved`. LR `1e-6`,
KL coefficient `0.02`, frozen SFT reference, 16 newly sampled reachable states
per update, eight distinct constrained guesses per state. The run sampled
160,000 training states and 1,280,000 actions across 10,000 optimizer updates.
These counts do not imply unique states. Runtime excluding initialization:
2,395.69 seconds (39.93 minutes).

All requested checkpoints were saved. The selection order is constrained
validation wins, lower average attempts, lower mean expected-survivor action
regret, then lower raw categorical KL from SFT on fixed SFT-sampled prefixes.
`checkpoint-ranking.json` retains the full ordered ranking.

#### Gameplay and checkpoint selection

All gameplay results use the same 72 validation secrets; no test secrets were
evaluated. Attempts are averaged over all games. KL is in nats per token on a
fixed reference-prefix panel, not full-game KL.

| Updates | Constrained wins | Raw wins | Average attempts | Mean solver regret | Fixed-prefix KL |
| ---: | ---: | ---: | ---: | ---: | ---: |
| 0 | 59/72 | 57/72 | 3.916667 | 0.043509 | 0 |
| 100 | 59/72 | 57/72 | 3.916667 | 0.039029 | 0.000009869 |
| **250** | **59/72** | **57/72** | **3.916667** | **0.038610** | **0.000031265** |
| 500 | 59/72 | 57/72 | 3.930556 | 0.038473 | 0.000102517 |
| 1,000 | 58/72 | 57/72 | 3.986111 | 0.037937 | 0.000366193 |
| 2,500 | 56/72 | 54/72 | 4.083333 | 0.031685 | 0.001407333 |
| 5,000 | 58/72 | 54/72 | 4.041667 | 0.028293 | 0.003693624 |
| 10,000 | 57/72 | 52/72 | 4.069444 | 0.010868 | 0.010024694 |

**Selected: update 250.** It ties the baseline on wins and attempts but has lower
regret, so the updated ranking selects a trained checkpoint instead of keeping
the baseline. Update 500 has slightly lower regret but loses on attempts;
later checkpoints lose on wins. `best.pt` was verified tensor-for-tensor equal
to `update-250.pt`.

#### Sampled-action diagnostics at every checkpoint

These are not training-batch averages or greedy gameplay rewards. They use a
fixed 128-state reachable panel sourced only from validation secrets, with
eight newly decoded distinct guesses per state (1,024 actions/checkpoint).
The same evaluation RNG seed is isolated from the training RNG. The panel is
saved in `diagnostic-states.json`. Reduction reward excludes the solve bonus.
Group std is the mean population standard deviation within each eight-action
group. Identical groups have exactly equal rewards across all eight guesses.

| Updates | Mean reward | Log-reduction reward | Immediate solves | Mean group std | Identical groups |
| ---: | ---: | ---: | ---: | ---: | ---: |
| 0 | 1.647374 | 1.427647 | 4.3945% | 0.969318 | 15.6250% |
| 100 | 1.647916 | 1.428190 | 4.3945% | 0.968761 | 15.6250% |
| 250 | 1.647013 | 1.427286 | 4.3945% | 0.965964 | 15.6250% |
| 500 | 1.642042 | 1.422315 | 4.3945% | 0.966993 | 15.6250% |
| 1,000 | 1.646313 | 1.426587 | 4.3945% | 0.960858 | 15.6250% |
| 2,500 | 1.642621 | 1.422895 | 4.3945% | 0.963704 | 14.8438% |
| 5,000 | 1.644357 | 1.419748 | 4.4922% | 0.975651 | 14.8438% |
| 10,000 | 1.648655 | 1.419163 | 4.5898% | 0.992548 | 14.0625% |

Policy entropy below is categorical token entropy summed over the five sampled
prefixes of each group's first draw, then averaged over states. It estimates
action entropy before within-group exclusions; it is not an exhaustive
dictionary-word entropy calculation. Diversity counts distinct sampled words
across all 1,024 panel actions, not the forced eight unique guesses per group.
Raw/constrained token entropies, conditional all-draw entropy, first-draw
diversity, empirical word-frequency entropy, and current-policy-prefix KL
are also recorded in each `validation-<update>.json`.

| Updates | First-draw action entropy (nats) | Unique sampled words / 1,024 actions |
| ---: | ---: | ---: |
| 0 | 0.739666 | 430 |
| 100 | 0.738982 | 430 |
| 250 | 0.748169 | 430 |
...
The run does not improve validation win count. The selected early checkpoint
improves the solver-regret tie-break without changing wins or average attempts.
At 10k, solver regret is lower still, but constrained wins fall from 59 to 57
and raw wins from 57 to 52. Mean panel reward is nearly flat; immediate solves
rise from 45 to 47 out of 1,024 actions while log-reduction reward declines.
Lower expected-survivor regret is not sufficient evidence of better gameplay.
No reward or hyperparameters were changed during this run.

Verification: 29 focused tests passed, including checkpoint-ranking precedence,
reward/solve-bonus decomposition, and raw-versus-constrained entropy/KL.
A two-update CUDA smoke exercised all diagnostics before the full run.
Completion checks verified all eight saved checkpoint step markers, all 10,000
training metric records, finite panel diagnostics, reward decomposition at every
checkpoint, and ranking consistency with the selected weights. Throwaway smoke
artifacts were removed. Full artifacts remain under `runs/grpo-dev-dense/`.

### 2026-09-23 — Complete-game GRPO from original SFT

Implemented and completed `train_grpo_games.py`, with stochastic full-game
sampling in `grpo_rollouts.py` and the whole-trajectory objective in
`grpo_trajectory_loss.py`.

```bash
uv run --with-requirements requirements.txt python train_grpo_games.py \
  --updates 10000 --output-dir runs/grpo-games-dev
```

Initialization and frozen reference both use the original 7,162,403-parameter
SFT checkpoint, not the one-guess update-250 checkpoint or DPO. Each optimizer
step draws 16 secrets uniformly with replacement from the training split and
samples eight independent stochastic constrained complete games per secret.
All eight group members start from the same empty board and actual hidden
secret. Duplicate trajectories and repeated legal guesses are allowed.

The terminal reward is exactly `7 - attempts` for a solve within six turns,
otherwise zero: solves on turns 1–6 receive 6, 5, 4, 3, 2, 1. There is no
candidate-reduction reward. Policy log-probabilities are summed across every
generated letter of every guess in the game; feedback, control tokens, and
padding are excluded. The whole-game likelihood ratio receives a group-relative
advantage, with no policy-log-probability length averaging. KL remains the exact
raw-vocabulary categorical KL averaged over generated-letter prefixes.

Configuration remained LR `1e-6`, KL coefficient `0.02`, gradient clipping
`1.0`, one optimizer step per fresh group batch, and at most six turns.
The run sampled 160,000 secret groups and 1,280,000 complete training games
across 10,000 updates. Runtime excluding initialization: 853.97 seconds
(14.23 minutes). All eight scheduled checkpoints were saved.

#### Greedy validation gameplay

Same 72 validation secrets as previous experiments; no test evaluation.
Average attempts includes failures. Fixed-prefix KL is raw categorical KL
per generated letter on the same frozen-SFT rollout prefixes at every checkpoint.

| Updates | Constrained wins | Average attempts | Raw wins | Mean solver regret | Fixed-prefix KL |
| ---: | ---: | ---: | ---: | ---: | ---: |
| **0 — selected** | **59/72** | **3.916667** | **57/72** | **0.043509** | **0** |
| 100 | 59/72 | 3.986111 | 57/72 | 0.035736 | 0.000239494 |
| 250 | 57/72 | 4.069444 | 55/72 | 0.033927 | 0.001187666 |
| 500 | 56/72 | 4.069444 | 55/72 | 0.027698 | 0.004010562 |
| 1,000 | 54/72 | 4.166667 | 52/72 | 0.023718 | 0.009351396 |
| 2,500 | 52/72 | 4.277778 | 49/72 | 0.020350 | 0.025480041 |
| 5,000 | 52/72 | 4.263889 | 50/72 | 0.023615 | 0.044010963 |
| 10,000 | 49/72 | 4.416667 | 47/72 | 0.035764 | 0.060933083 |

**Selection remains update 0, the original SFT.** Update 100 ties its win count
but loses on average attempts; all later checkpoints lose on wins. Selection
retains the prior order: wins, lower attempts, lower regret, lower fixed-prefix
KL. This is not an unconditional baseline tie rule. The saved `best.pt` was
verified tensor-for-tensor equal to the original SFT weights.

#### Stochastic validation rollout diagnostics

At every checkpoint, eight independent complete games are sampled for each
of the 72 validation secrets: 576 games total. The evaluation RNG is reset to
the same seed and isolated from training. Full per-game traces are saved in
each `validation-<update>.json`. Winning guesses below average only successful
games. Group std is the mean population reward std within each same-secret group.
Current-prefix KL uses the current model's sampled games, unlike fixed-prefix
KL in the ranking table above.

| Updates | Sampled wins | Win fraction | Winning guesses | Mean reward | Group reward std | Identical groups | Current-prefix KL |
| ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 0 | 456/576 | 79.1667% | 3.583333 | 2.704861 | 0.676413 | 22.2222% | 0 |
| 100 | 462/576 | 80.2083% | 3.586580 | 2.737847 | 0.689927 | 20.8333% | 0.000241891 |
| 250 | 454/576 | 78.8194% | 3.555066 | 2.715278 | 0.684536 | 25.0000% | 0.001197957 |
| 500 | 446/576 | 77.4306% | 3.547085 | 2.673611 | 0.671152 | 26.3889% | 0.004056584 |
| 1,000 | 450/576 | 78.1250% | 3.620000 | 2.640625 | 0.623999 | 31.9444% | 0.009588429 |
| 2,500 | 427/576 | 74.1319% | 3.625293 | 2.501736 | 0.574840 | 31.9444% | 0.027451463 |
| 5,000 | 408/576 | 70.8333% | 3.632353 | 2.385417 | 0.565036 | 37.5000% | 0.052056585 |
| 10,000 | 402/576 | 69.7917% | 3.716418 | 2.291667 | 0.382981 | 52.7778% | 0.074998111 |

#### Training reward variation and outcome

The training split was already nearly perfectly solved. Aggregating the first
and last 100 updates (12,800 fresh sampled games per window):

| Training metric | First 100 updates | Last 100 updates |
| --- | ---: | ---: |
| Sampled win fraction | 99.9766% | 99.9922% |
| Mean guesses among winners | 3.154333 | 3.030627 |
| Mean trajectory reward | 3.844766 | 3.969063 |
| Mean within-group reward std | 0.216107 | 0.047882 |
| Identical-reward group rate | 51.9375% | 88.1250% |

Training reward improves while held-out gameplay deteriorates. Most late
training groups provide no reward advantage because every member receives
the same reward; KL regularization still applies. The final training batch
had 100% identical-reward groups. No trained checkpoint improves the specified
greedy validation ranking over the original SFT. The brief sampled-validation
improvement at update 100 does not establish a greedy validation improvement.
Reward and hyperparameters were not changed during the run.

Verification: 40 focused tests passed, covering shared secret/start, exact real
feedback histories, solve/six-turn termination, the complete reward ladder,
legal repeat behavior, sampled-versus-teacher-forced whole-game likelihood,
late-guess policy gradients, padding/control exclusion, group-local advantages,
and KL with zero reward signal. A two-update CUDA smoke changed all 54 parameter
tensors before the proper run. Post-run checks verified all 10,000 update records,
near-one on-policy likelihood ratios, all eight checkpoint step markers,
same-secret grouping and terminal rewards in every saved validation rollout,
diagnostic aggregates recomputed from those traces, and ranking/weight provenance.
Smoke artifacts were removed; full results remain under `runs/grpo-games-dev/`.

### 2026-09-24 — Reachable-state continuations and matched KL anchors

Implemented reachable-state training in `train_grpo_games.py`, corpus sampling
in `grpo_corpus.py`, and mixed-depth continuation support in `grpo_rollouts.py`.
Completed three 1,000-update development runs from the original 7.2M SFT:
KL beta **0.02, 0.05, 0.10**, with everything else fixed.

#### Corpus and controlled setup

`grpo_corpus.py` scanned all 1,000,000 records from
`data/wordle-v2-diverse-1m/examples.jsonl.gz`. Of these, 888,230 nonempty states
belong to training source secrets and 111,769 to validation source secrets;
the one canonical empty board was excluded. Uniform reservoir sampling with
seed 20260923 selected 100,000 training states and a separate 512-state validation
panel. Both use histories of 1–5 real, unsolved guesses. No SFT target or
empty-board sampling weight affects this corpus.

Training states cover all 575 training secrets; the validation panel covers all
72 validation secrets. No test-secret states are used. This restriction applies
to the hidden source secret, not to which dictionary words may be guessed or
remain consistent with an observable history.

| Guesses already used | Training states | Validation states |
| ---: | ---: | ---: |
| 1 | 3,542 | 17 |
| 2 | 30,393 | 156 |
| 3 | 27,708 | 154 |
...
Corpus files and provenance are under `data/grpo-continuations/`.
Source SHA-256:
`4fff02cb7c8b2a4488a9f9b68763e5d625b909201c17ad4e8be9f3bef1f483a4`.
Training corpus SHA-256:
`f48f70923b3d22f18dc5d914b76baf450e12563de8f13533b4df3ac0c667194e`.

Each update uniformly samples 16 states with replacement from this fixed corpus,
then samples eight independent dictionary-constrained continuations per state.
Group members share their history and secret; duplicate continuations and
repeated guesses remain legal. Games end on a solve or six **total** guesses.
Reward is `7 - total_guesses_used` if solved, otherwise zero. Supplied history
counts toward the turn limit and reward, but only newly generated letters enter
the summed continuation likelihood and raw categorical prefix-KL regularizer.

Fixed settings: original SFT initialization/reference, seed 0, LR `1e-6`,
temperature 1, gradient clipping 1.0, one optimizer step per fresh batch, and
checkpoints at 0, 100, 250, 500, 1,000. Per run: 16,000 state draws, 14,781 distinct
states visited, and 128,000 continuations. This is a short development comparison,
not a full pass over all 100,000 corpus states. All 16,000 source-state draws
match exactly across betas. Configurations differ only in beta and output path.
Each run took approximately 128.6 seconds excluding initialization.

#### Held-out continuation results

At each checkpoint, sample eight continuations from each of the fixed 512
validation states: 4,096 finishes. RNG is reset and isolated from training.
Winning attempts below include the supplied history. The three coefficients
produce the same aggregate win counts, rewards, and winning-attempt averages
at each listed checkpoint, despite distinct learned weights.

| Updates | Wins / 4,096 | Win rate | Winning total attempts | Mean terminal reward |
| ---: | ---: | ---: | ---: | ---: |
| 0 | 2,788 | 68.0664% | 4.786944 | 1.506348 |
| 100 | 2,793 | 68.1885% | 4.789832 | 1.507080 |
| 250 | 2,785 | 67.9932% | 4.783483 | 1.507080 |
| 500 | 2,777 | 67.7979% | 4.782859 | 1.503174 |
| 1,000 | 2,773 | 67.7002% | 4.778218 | 1.504150 |

At update 1,000:

| Beta | Continuation raw-prefix KL | Group reward std | Identical-reward groups | Fixed SFT empty-board-prefix KL |
| ---: | ---: | ---: | ---: | ---: |
| 0.02 | 0.001220186 | 0.378119 | 42.3828% | 0.000814745 |
| 0.05 | 0.001217822 | 0.376995 | 42.3828% | 0.000812640 |
| 0.10 | 0.001211061 | 0.378119 | 42.3828% | 0.000808558 |

Baseline group std is 0.374806, with 43.1641% identical-reward groups.
Increasing beta fivefold reduces measured final continuation-prefix KL by only
0.75%. These short runs show no separation in final continuation wins or reward
among the tested anchors. This is one seed, not evidence of general equivalence.

#### Ordinary greedy gameplay and checkpoint selection

All three betas also produce the same ordinary greedy validation metrics:

| Updates | Constrained wins / 72 | Average attempts, including failures | Raw wins / 72 |
| ---: | ---: | ---: | ---: |
| **0 — selected** | **59** | **3.916667** | **57** |
| 100 | 59 | 3.916667 | 57 |
| 250 | 59 | 3.930556 | 57 |
| 500 | 57 | 3.930556 | 56 |
| 1,000 | 57 | 3.972222 | 56 |

The original SFT remains selected in every run under the unchanged ranking:
constrained wins, lower average attempts, lower solver regret, lower fixed-prefix
KL. Update 100 does not beat the baseline ranking; later checkpoints lose on
attempts or wins. All three `best.pt` files equal the original SFT tensor-for-tensor.
The slight early sampled-continuation improvement is not a greedy-gameplay gain.

Training diagnostics, aggregated over 100-update windows, are also identical
across coefficients:

| Metric | First 100 updates | Last 100 updates |
| --- | ---: | ---: |
| Sampled continuation win fraction | 91.5625% | 92.6641% |
| Winning total attempts | 4.569625 | 4.629121 |
| Mean terminal reward | 2.225313 | 2.196953 |
| Mean group reward std | 0.160565 | 0.170606 |
| Identical-reward groups | 65.0625% | 63.1250% |

These windows contain different random histories/depths, so their reward
difference is not a paired estimate of learning. Unlike the prior long empty-board
run, these short continuation runs do not show reward-group collapse over time.
They nevertheless fail to improve held-out gameplay, and the tested beta range
does not materially alter that outcome.

#### Reproduction, artifacts, and verification

Use the corpus/training commands in README, varying only `--kl-beta` and
`--output-dir`. Artifacts:

- `runs/grpo-continuations-beta-002/`
- `runs/grpo-continuations-beta-005/`
- `runs/grpo-continuations-beta-010/`
- `runs/grpo-continuations-comparison.json`

46 focused tests passed, including final-turn reward 1 versus failure 0,
mixed-depth continuation limits, likelihood excluding supplied history, truthful
feedback, split isolation, and deterministic corpus sampling. A two-update CUDA
smoke changed all 54 parameter tensors. Post-run verification checked every
saved rollout's history, group, total attempts, terminal result, and reward;
recomputed diagnostic aggregates; checked all 3,000 update records and matched
state orders; and verified all 15 checkpoint markers, finite weights, and selected
baseline weights. Smoke artifacts were removed. No test secrets were evaluated.

### 2026-09-24 — Token-level GRPO correction and fixed-beta LR sweep

**Decision: stop this LR sweep without extending GRPO.** All three rates improve
fixed-panel training reward while final held-out continuation reward falls.
Neither faster rate produces a credible early held-out gain. The policy can
move and learn on training secrets; optimization strength alone does not explain
the lack of generalization. This is observed train/validation divergence
consistent with RL overfitting, not proof that every terminal-reward method fails.

#### Correcting the objective before the sweep

Inspection found that the previous implementation used
`exp(sum(new_token_logps) - sum(old_token_logps))` and one clipped surrogate per
trajectory. Replaced it with stored per-token behavior log-probabilities and
token-level clipping:

```
A_i = group-relative terminal-reward advantage for trajectory i
r_it = exp(new_token_logp_it - old_token_logp_it)
policy_loss = -mean_i mean_generated_t min(r_it A_i, clip(r_it, 0.8, 1.2) A_i)
loss = policy_loss + 0.10 * mean_i mean_generated_t KL(policy_it || SFT_it)
```

Each rollout has equal weight, irrespective of generated token count. Both
reward and raw categorical KL first average within a rollout and then across
rollouts. Supplied history, feedback, control tokens, and padding are excluded.
Summed trajectory likelihoods remain diagnostic only. Terminal reward stays
`7 - total_guesses_used` on solve and zero on failure; supplied history counts.

The earlier experiments used one fresh on-policy batch per optimizer step,
so their pre-update whole-trajectory ratios were near one rather than exploding.
Nevertheless, the summed log-probability gradient automatically weighted longer
continuations more heavily. The correction removes that structural weighting;
the new results are not a pure LR comparison against the old objective.

#### Fixed setup and paired reward tracking

Three fresh runs from original 7.2M SFT: LR `1e-6`, `3e-6`, `1e-5`, beta fixed
at `0.10`, 1,000 updates each. Same 100,000-state training corpus, seed 0, exact
state-draw order, eight independent constrained completions per state,
16 states per update, temperature 1, AdamW without weight decay, and global
gradient norm clipping at 1.0. No test secrets are used.

Each run draws 16,000 groups / 128,000 continuations and visits the same 14,781
distinct source states. Checkpoints: 0, 100, 250, 500, 1,000.
Each run took approximately 143 seconds excluding initialization.

Every checkpoint evaluates two fixed panels:

- Training: 512 states sampled from the training corpus with seed 2000003,
  covering 337 training secrets.
- Held-out: the existing 512-state panel covering all 72 validation secrets.
- Eight stochastic continuations per state: 4,096 outcomes per panel.
- Reset, isolated evaluation RNG; evaluation does not change training draws.

Unlike reward averages over changing minibatches, these paired panels hold
history difficulty and remaining-turn distributions fixed across checkpoints.

#### Training and held-out continuation reward together

| LR | Updates | Fixed training reward | Held-out reward |
| ---: | ---: | ---: | ---: |
| All | 0 | 2.229248 | 1.506348 |
| 1e-6 | 100 | 2.229736 | 1.506592 |
| 1e-6 | 250 | 2.231445 | 1.507080 |
| 1e-6 | 500 | 2.234131 | 1.503662 |
| 1e-6 | 1,000 | 2.238770 | 1.500000 |
| 3e-6 | 100 | 2.230469 | 1.506592 |
| 3e-6 | 250 | 2.233154 | 1.506592 |
| 3e-6 | 500 | 2.243164 | 1.500977 |
| 3e-6 | 1,000 | 2.250244 | 1.496094 |
| 1e-5 | 100 | 2.237305 | 1.503906 |
| 1e-5 | 250 | 2.249023 | 1.502686 |
| 1e-5 | 500 | 2.255859 | 1.500244 |
| 1e-5 | 1,000 | 2.268066 | 1.476562 |

Higher LR increases the training gain, not held-out gain. At `1e-5`, every
evaluated trained checkpoint has lower held-out continuation reward than SFT.

Paired changes at update 1,000, with 95% percentile intervals from 10,000
bootstrap resamples of **secret clusters**, retaining all state/rollout
observations for a resampled secret:

| LR | Training reward change [95% interval] | Held-out reward change [95% interval] |
| ---: | --- | --- |
| 1e-6 | +0.009521 [+0.004071, +0.015291] | -0.006348 [-0.020309, +0.007952] |
| 3e-6 | +0.020996 [+0.011622, +0.030452] | -0.010254 [-0.031827, +0.011654] |
| 1e-5 | +0.038818 [+0.023857, +0.053393] | -0.029785 [-0.064533, +0.004225] |

The training gains are clear on these panels. Held-out decrease intervals still
overlap zero, so the evidence supports **no demonstrated held-out improvement**,
not a statistically established population loss for every LR.
This is one training seed; intervals do not include training-seed variation.

The tiny early peaks do not justify continuation:

- `1e-6`, update 250: held-out change +0.000732, interval [-0.009466, +0.010649].
- `3e-6`, update 100: +0.000244, interval [-0.011719, +0.012371].
- `1e-5`: best held-out reward remains update 0.

These early intervals are exploratory, without adjustment for inspecting
multiple checkpoints. Neither faster rate satisfies a credible train-up /
held-out-up continuation criterion.

#### Greedy gameplay, policy movement, and numerical stability

At update 1,000; average attempts includes failures. Continuation KL uses
current sampled held-out prefixes with the equal-rollout reduction.

| LR | Held-out continuation wins / 4,096 | Continuation KL | Fixed SFT-prefix KL | Greedy constrained wins / 72 | Average attempts | Raw wins / 72 |
| ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| Original SFT | 2,788 | 0 | 0 | 59 | 3.916667 | 57 |
| 1e-6 | 2,762 | 0.001099989 | 0.000736730 | 57 | 3.986111 | 56 |
| 3e-6 | 2,754 | 0.006234769 | 0.004179762 | 58 | 3.972222 | 56 |
| 1e-5 | 2,732 | 0.020750370 | 0.013400533 | 56 | 4.013889 | 53 |

All three greedy `best.pt` files remain the original SFT, verified
tensor-for-tensor. Separate reward-ranked `best-continuation.pt` files select
updates 250, 100, and 0 respectively; the small selected gains above are not
claimed as genuine improvements.

All losses, gradient norms, and saved weights remained finite. Maximum
pre-clipping parameter-gradient norms were 1.205608, 1.216481, and 1.197920;
gradient clipping activated on 10, 11, and 14 updates respectively. This is
policy drift and train/validation divergence, not obvious numerical instability.

Every pre-update token clipping fraction was zero, as expected for one update
per fresh on-policy batch. Post-update sampled-token ratio ranges across each run:

| LR | Minimum ratio | Maximum ratio | Maximum per-batch clipping fraction |
| ---: | ---: | ---: | ---: |
| 1e-6 | 0.940742 | 1.053315 | 0 |
| 3e-6 | 0.831521 | 1.163548 | 0 |
| 1e-5 | 0.452066 | 1.568556 | 1.875% |

Post-update clipping fractions are diagnostics, not a second policy update.
Token PPO clipping does not bound the ensuing Adam step when the only gradient
was evaluated at on-policy ratios near one.

#### Advantage and parameter-gradient contribution by length

`length_diagnostics` is logged at update 1 and every 100 updates. It measures
actual pre-clipping parameter-gradient L2 norms for reward-only and combined
losses, by both generated guesses and initially remaining guesses. Each bucket
retains the global batch denominator. Its gradient norm is not an additive
share: vector contributions can cancel.

For `1e-5`, aggregated over the 11 diagnostic batches (1,408 rollouts):

| Generated guesses | Rollouts | Mean absolute advantage | Per-token weight within rollout | Mean reward-gradient contribution L2 | Mean combined contribution L2 |
| ---: | ---: | ---: | ---: | ---: | ---: |
| 1 | 789 | 0.110523 | 0.200000 | 0.303332 | 0.301211 |
| 2 | 542 | 0.320309 | 0.100000 | 0.283897 | 0.282332 |
| 3 | 74 | 1.408147 | 0.066667 | 0.189656 | 0.188929 |
| 4 | 3 | 2.645751 | 0.050000 | 0.031802 | 0.031738 |
| 5 | 0 | — | 0.040000 | 0 | 0 |

Longer sampled completions have larger advantages in magnitude, but no automatic
extra token-count multiplier. The sparse four-/five-guess bins cannot support a
strong distributional claim. Regression tests separately verify that duplicating
a rollout's token contributions changes neither reward/KL loss nor parameter
gradients, and that compensating token ratios clip independently rather than
collapsing into a trajectory product.

#### Artifacts, verification, and stopping decision

- `runs/grpo-token-lr-1e6/`
- `runs/grpo-token-lr-3e6/`
- `runs/grpo-token-lr-1e5/`
- `runs/grpo-token-lr-comparison.json`

50 focused tests passed. A two-update CUDA smoke changed all 54 parameter
tensors; restoring a token-level checkpoint, Adam state, and Python/CPU/CUDA
RNG states reproduced the uninterrupted second update and weights exactly.
Post-run checks validated all 3,000 update records, matched state sequences,
33 diagnostic batches against actual training gradient norms, saved panel and
training-trace rewards, all 15 checkpoint markers and finite weights, and both
checkpoint selection policies. Smoke artifacts were removed.

No LR was extended. The simple terminal-reward setup does not beat the original
SFT on these development experiments, even with stronger optimization and the
length-weighting defect removed. The next investigation should target
credit assignment rather than another unconstrained LR/beta search.

### 2026-09-24 — One-guess exact expected-information GRPO

Implemented and ran 1,000 updates from the **original 7,162,403-parameter SFT**,
not an RL checkpoint. Learning rate `3e-6`, frozen-original-SFT raw KL beta
`0.10`, 16 states/update, two-state gradient microbatches. Each state receives
48 stochastic policy proposals and 16 random legal proposals, deduplicated and
refilled to 64 distinct words.

For all dictionary answers consistent with the history, with `N` candidates:

```text
expected_after = sum(feedback_bucket_size ** 2) / N
reward = log(N / expected_after) + 2 * indicator(guess in candidates) / N
```

Feedback partitions use duplicate-letter-correct Wordle feedback. Reward never
uses the source secret as the single sampled answer. Group standardization is
over all 64 rewards. Only the five new guess-letter positions contribute loss;
token PPO ratios and KL use the original dictionary mask. Exclusion masks are
only for duplicate refill. The mixed, deduplicated, equally weighted candidates
define a **ranking surrogate**, not an unbiased on-policy PPO estimator.
Single optimizer updates per freshly sampled batch leave pre-update ratios
near one and clipping inactive; the nonzero gradient comes from the
group-relative advantages. Prefix-logit caches live only within one proposal
batch and cannot survive a model update.

#### State mixture and actual coverage

Scanned the existing 1M corpus and retained 20,199 training-source states and
2,542 validation-source states, reservoir-sampled per difficulty/depth/behavior
cell. Existing behaviors are entropy, simple, partly-random, poor, and random.
Fresh SFT rollouts supply model states initially; 256 current-policy games
refresh that pool every 100 updates. Nonempty sampling chooses an available
difficulty bucket uniformly, then an available depth, then an available behavior.
Unavailable cells are reported, not synthesized.

Actual 16,000 training groups:

- Empty opening: **1,200 / 16,000 = 7.5%**.
- Candidate buckets, excluding openings: `1–2`: 2,926; `3–5`: 3,034;
  `6–20`: 2,952; `21–100`: 2,916; `101+`: 2,972.
- History depths 1–5: 2,953 / 2,979 / 2,884 / 3,034 / 2,950.
- Behaviors: model 1,090; entropy 770; partly-random 3,211; poor 4,469;
  random 2,996; simple 2,264.
- Only **one** group had identical rewards. The initial 48 policy draws
  contained only **4.54 distinct guesses per state on average**, making refill
  essential. Forced proposal diversity must not be confused with policy entropy.

**Evaluation limitation:** training-source secrets exclude validation and test
secrets, but full-dictionary candidate sets and exact counterfactual rewards
include validation/test answer words. This is held-out-source evaluation, not
fully unseen answer supervision. No test gameplay was evaluated.

#### Fixed-panel reward and greedy gameplay

Fixed training and validation panels each contain 400 states, including model
histories and 7.5% openings. Stochastic reward uses eight independent policy
guesses per state, without forced diversity or random proposals. Greedy
gameplay covers all 72 validation secrets.

| Update | Train stochastic reward | Validation stochastic reward | Validation greedy reward | Oracle reward regret | Greedy wins | Attempts/game | Raw wins | Validation raw KL |
|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 0 | 2.484031 | 2.418272 | 2.456066 | 0.226682 | 59/72 | 3.916667 | 57/72 | 0 |
| 100 | 2.489230 | 2.421270 | 2.459563 | 0.223185 | 59/72 | 3.986111 | 56/72 | 0.001203 |
| 250 | 2.491998 | 2.424677 | 2.463564 | 0.219185 | **60/72** | **3.875000** | **58/72** | 0.018804 |
| 500 | 2.490850 | **2.432308** | **2.469072** | **0.213677** | 57/72 | 4.097222 | 53/72 | 0.042573 |
| 1000 | 2.479228 | 2.429615 | 2.458654 | 0.224095 | 54/72 | 4.083333 | 50/72 | 0.123534 |

At update 500, validation stochastic reward improves by **0.014036**.
A descriptive paired-state normal 95% interval is `[0.003849, 0.024223]`;
related/repeated histories and a single training seed limit its interpretation.
The greedy reward gain comes from the solve-probability bonus: information-only
reward decreases from **2.204167 to 2.200338**, while solve probability increases
from **0.125949 to 0.134367**.

Validation stochastic reward by candidate bucket at update 0 → 500:

| Candidates | Panel states | Original SFT | Update 500 |
|---|---:|---:|---:|
| 1–2 | 70 | 0.991550 | 1.043336 |
| 3–5 | 65 | 1.509190 | 1.527229 |
| 6–20 | 70 | 1.925094 | 1.947044 |
| 21–100 | 79 | 2.860968 | 2.862216 |
| 101+ (including openings) | 116 | 3.784740 | 3.777687 |

The best information-reward checkpoint is update **500**; the best gameplay
checkpoint is update **250**. The latter wins only one more validation game
than original SFT. Continued training to 1,000 degrades gameplay and increases
KL. This run provides evidence of a small reward improvement, **not a robust
general gameplay improvement or improved information gathering**.

#### Artifacts and verification

- `data/grpo-information/{train,validation}.jsonl` and corpus manifest.
- `runs/grpo-information-dev/comparison.json`.
- Fixed panels, per-checkpoint evaluations/strata, 1,000 update metrics,
  proposal traces, model-state refreshes, and coverage in the same run directory.
- `checkpoints/best-information.pt`: update 500.
- `checkpoints/best-gameplay.pt`: update 250.

51 focused tests passed. Two-update CUDA smoke runs exercised training,
evaluation, and model-state refresh. Cached and uncached first-update proposals
and rewards matched exactly. Independent feedback-partition recomputation
verified 2,048 smoke rewards and all 5,120 saved full-run proposal rewards,
including candidate sets and the 48/16 unique-guess contract. All 1,000 gradient
norms were finite and positive. All five scheduled checkpoints had finite
weights and correct update/provenance markers; update zero exactly matched
original SFT, and every trained checkpoint changed all 54 parameter tensors.
Both best-checkpoint markers matched their recorded selections. Smoke artifacts
were removed.
